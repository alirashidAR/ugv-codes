...
# -----------------------------
is_running = False
turn_override_active = False
turn_override_end_time = 0
...
def process_and_drive(frame):
    global last_error
    global turn_override_active, turn_override_end_time

    h, w = frame.shape[:2]
    img_center = w // 2

    # =============================
    # HARD LEFT OVERRIDE MODE
    # =============================
    if turn_override_active:
        if time.time() < turn_override_end_time:
            # HARD pivot left
    ...
    if l_valid and r_valid:
        ...
        # brief memory instead of hard stop
...
def turn_left():
    global turn_override_active, turn_override_end_time
    turn_override_active = True
    turn_override_end_time = time.time() + 0.03
    return "Hard left override activated"
